...
func TestMigrationV50PiPreservesCurrentStore(t *testing.T) {
	var f struct {
		Cases []struct {
	...
	for _, c := range f.Cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			defer pool.Put(conn)
			before := sqlitemigration.Schema{Migrations: dbSchema.Migrations[:49], MigrationOptions: dbSchema.MigrationOptions[:49]}
