...
func TestMigrationV52LegacyCaptureIncomplete(t *testing.T) {
	...
	defer s.Close()
	c := capture(t, s, id)
	if c.Status != ingest.ContentCaptureIncomplete || c.SourceAuthority != ingest.ContentSourceNone || c.CaptureRevision != "legacy-preview-only-v52" || c.FailureCode != "legacy_preview_only" || c.FullCaptureSHA256 != "" || c.PublicationCaptureRevision != 0 {
