...
func TestMigrationV53PiPreservesCurrentStore(t *testing.T) {
	...
	for _, c := range f.Cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			if err != nil || c.FullRepetitions <= 0 || c.FullPrefix == "" || c.FullTail == "" {
			...
			written := oldDB.IndexSessionEntryBatch(ctx, []ingest.SessionEntryWrite{{SessionID: sid, Entries: entries, RequireFullContent: true}})
