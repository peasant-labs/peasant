...
func TestMigrationV53PiPreservesCurrentStore(t *testing.T) {
	...
	for _, c := range f.Cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			written := oldDB.IndexSessionEntryBatch(ctx, []ingest.SessionEntryWrite{{SessionID: sid, Result: indexformat.V1{Entries: entries}, RequireFullContent: true, CaptureRevision: revision, IndexVersion: 1, IndexerVersion: ingest.HarvesterVersionRegistry[ingest.HarnessClaudeCode].IndexerVersion, IndexedAtMs: 4}})
