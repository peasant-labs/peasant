...
// TestScannerTreeSource_UnnamedRemoteLessProjectUsesShortPathLabel proves a
// project whose discovery has neither a Git remote nor a project name renders
// its shortest distinct path suffix, never the removed placeholder.
func TestScannerTreeSource_UnnamedRemoteLessProjectUsesShortPathLabel(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	clone := filepath.Join(root, "labs", "notes")
	if err := os.MkdirAll(clone, 0o755); err != nil {
		t.Fatalf("create unnamed project directory %q: %v", clone, err)
	}
	listings := []ftue.SessionListing{
		{Harness: "claude-code", Branch: "main", SessionID: "unnamed-single", WorkingDir: clone},
	}
	roots, err := kickstart.NewScannerTreeSource(listings).Load(context.Background())
	if err != nil {
		t.Fatalf("scanner load: %v", err)
	...
	if len(roots) != 1 {
		t.Fatalf("unnamed remote-less roots = %d, want 1", len(roots))
	}
	requireUnnamedPathLabel(t, roots[0], filepath.Join("labs", "notes"))
}

// TestScannerTreeSource_UnnamedRemoteLessLabelsWidenToStayDistinct proves two
// unnamed remote-less projects whose shortest suffixes collide widen together
// until every label is distinct.
func TestScannerTreeSource_UnnamedRemoteLessLabelsWidenToStayDistinct(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	cloneA := filepath.Join(root, "team-a", "shared", "app")
	cloneB := filepath.Join(root, "team-b", "shared", "app")
	...
	listings := []ftue.SessionListing{
		{Harness: "claude-code", Branch: "main", SessionID: "unnamed-a", WorkingDir: cloneA},
		{Harness: "claude-code", Branch: "main", SessionID: "unnamed-b", WorkingDir: cloneB},
	}
	roots, err := kickstart.NewScannerTreeSource(listings).Load(context.Background())
	if err != nil {
		t.Fatalf("scanner load: %v", err)
	...
	if len(roots) != 2 {
		t.Fatalf("unnamed duplicate-tail roots = %d, want 2", len(roots))
	}
	labels := map[string]bool{}
	for _, project := range roots {
		labels[project.Label] = true
		if strings.Contains(project.Label, "(unknown project)") || strings.Contains(project.Label, root) {
			t.Fatalf("unnamed duplicate-tail label = %q, want a relative path suffix with no placeholder", project.Label)
		}
	}
	for _, want := range []string{filepath.Join("team-a", "shared", "app"), filepath.Join("team-b", "shared", "app")} {
		if !labels[want] {
			t.Errorf("missing widened unnamed label %q; got %v", want, labels)
		}
	}
}

// TestScannerTreeSource_UnnamedLocalRemoteGitProjectUsesShortPathLabel proves a
// real Git worktree whose only remote is a local path (so no canonical remote
// label exists) and whose discovery name is empty renders its shortest path
// suffix.
func TestScannerTreeSource_UnnamedLocalRemoteGitProjectUsesShortPathLabel(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	clone := filepath.Join(root, "checkout", "notes")
	localRemote := filepath.Join(root, "origin.git")
	initGitProject(t, clone, localRemote)
	listings := []ftue.SessionListing{
		{Harness: "claude-code", GitRemote: localRemote, Branch: "main", SessionID: "unnamed-local-remote", WorkingDir: clone},
	}
	roots, err := kickstart.NewScannerTreeSource(listings).Load(context.Background())
	if err != nil {
		t.Fatalf("scanner load: %v", err)
	...
	if len(roots) != 1 {
		t.Fatalf("unnamed local-remote Git roots = %d, want 1", len(roots))
	}
	if identity := roots[0].Meta[settings.MetaProjectIdentity]; !strings.HasPrefix(identity, "repo:") {
		t.Fatalf("unnamed local-remote project identity = %q, want a Git repository cohort", identity)
	}
	requireUnnamedPathLabel(t, roots[0], filepath.Join("checkout", "notes"))
}

// TestScannerTreeSource_UnnamedLocalRemoteGitProjectWidensWithRemoteLessSibling
// proves an unnamed Git project with an unparseable local remote and an unnamed
// remote-less project share one widening group: equal shortest suffixes widen
// across both.
func TestScannerTreeSource_UnnamedLocalRemoteGitProjectWidensWithRemoteLessSibling(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	gitClone := filepath.Join(root, "team-a", "shared", "app")
	plainClone := filepath.Join(root, "team-b", "shared", "app")
	localRemote := filepath.Join(root, "origin.git")
	initGitProject(t, gitClone, localRemote)
	if err := os.MkdirAll(plainClone, 0o755); err != nil {
		t.Fatalf("create remote-less clone directory %q: %v", plainClone, err)
	}
	listings := []ftue.SessionListing{
		{Harness: "claude-code", GitRemote: localRemote, Branch: "main", SessionID: "unnamed-local-remote", WorkingDir: gitClone},
		{Harness: "claude-code", Branch: "main", SessionID: "unnamed-remote-less", WorkingDir: plainClone},
	}
	roots, err := kickstart.NewScannerTreeSource(listings).Load(context.Background())
	if err != nil {
		t.Fatalf("scanner load: %v", err)
	...
	if len(roots) != 2 {
		t.Fatalf("mixed unnamed duplicate-tail roots = %d, want 2", len(roots))
	}
	labels := map[string]bool{}
	for _, project := range roots {
		labels[project.Label] = true
		if strings.Contains(project.Label, "(unknown project)") || strings.Contains(project.Label, root) {
			t.Fatalf("mixed unnamed label = %q, want a relative path suffix with no placeholder", project.Label)
		}
	}
	for _, want := range []string{filepath.Join("team-a", "shared", "app"), filepath.Join("team-b", "shared", "app")} {
		if !labels[want] {
			t.Errorf("missing widened mixed unnamed label %q; got %v", want, labels)
		}
	}
}

// TestScannerTreeSource_NamedLocalRemoteGitProjectKeepsBareName proves a named
// project keeps its bare-name label when its remote is an unparseable local
// path.
func TestScannerTreeSource_NamedLocalRemoteGitProjectKeepsBareName(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	clone := filepath.Join(root, "checkout", "notes")
	localRemote := filepath.Join(root, "origin.git")
	initGitProject(t, clone, localRemote)
	listings := []ftue.SessionListing{
		{Harness: "claude-code", ProjectName: "notes", GitRemote: localRemote, Branch: "main", SessionID: "named-local-remote", WorkingDir: clone},
	}
	roots, err := kickstart.NewScannerTreeSource(listings).Load(context.Background())
	if err != nil {
		t.Fatalf("scanner load: %v", err)
	...
	if len(roots) != 1 {
		t.Fatalf("named local-remote Git roots = %d, want 1", len(roots))
	}
	if roots[0].Label != "notes" {
		t.Fatalf("named local-remote label = %q, want the bare project name %q", roots[0].Label, "notes")
	}
}

// TestScannerTreeSource_UnnamedCanonicalRemoteKeepsRemoteLabel proves a
// canonical remote names a project even when discovery produced no project
// name: the path fallback never replaces a remote label.
func TestScannerTreeSource_UnnamedCanonicalRemoteKeepsRemoteLabel(t *testing.T) {
	t.Parallel()
	root := t.TempDir()
	clone := filepath.Join(root, "checkout", "tool")
	initGitProject(t, clone, "git@github.com:acme/tool.git")
	listings := []ftue.SessionListing{
		{Harness: "claude-code", GitRemote: "git@github.com:acme/tool.git", Branch: "main", SessionID: "unnamed-canonical-remote", WorkingDir: clone},
	}
	roots, err := kickstart.NewScannerTreeSource(listings).Load(context.Background())
	if err != nil {
		t.Fatalf("scanner load: %v", err)
	...
	if len(roots) != 1 {
		t.Fatalf("unnamed canonical-remote roots = %d, want 1", len(roots))
	}
	if roots[0].Label != "github.com:acme/tool" {
		t.Fatalf("unnamed canonical-remote label = %q, want %q", roots[0].Label, "github.com:acme/tool")
	}
}

// requireUnnamedPathLabel asserts an unnamed project row renders exactly the
// expected relative path suffix.
func requireUnnamedPathLabel(t *testing.T, project *kit.TreeNode, want string) {
	t.Helper()
	if project.Label != want {
		t.Fatalf("unnamed project label = %q, want the shortest path suffix %q", project.Label, want)
	}
}
