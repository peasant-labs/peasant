...
type redactionProfileFixture struct {
	CustomPatterns       []config.CustomPattern  `yaml:"customPatterns"`
	DeclaredPatterns     *[]config.CustomPattern `yaml:"declaredPatterns"`
	XDGDataHome          string                  `yaml:"xdgDataHome"`
	XDGConfigHome        string                  `yaml:"xdgConfigHome"`
	XDGStateHome         string                  `yaml:"xdgStateHome"`
	CustomCategory       redact.Category         `yaml:"customCategory"`
	DuplicateCategory    bool                    `yaml:"duplicateCategory"`
	ZeroCount            bool                    `yaml:"zeroCount"`
	ReportCounts         map[string]int          `yaml:"reportCounts"`
	ReportCategories     []string                `yaml:"reportCategories"`
	ReportTotal          int                     `yaml:"reportTotal"`
...
func (r *profileReportRedactor) Report() redact.RedactionReport {
	...
	if r.fixture.ZeroCount {
		report.Counts["email"] = 0
	}
	if r.fixture.DuplicateCategory {
		report.Categories = append(report.Categories, report.Categories...)
	}
	if r.fixture.ReportCounts != nil {
		report.Counts = maps.Clone(r.fixture.ReportCounts)
		report.Categories = append([]string(nil), r.fixture.ReportCategories...)
		report.TotalRedactions = r.fixture.ReportTotal
	}
...
func TestPipelineRedactionProfile(t *testing.T) {
	...
	for _, fixture := range loadRedactionProfileFixtures(t) {
		t.Run(fixture.Name, func(t *testing.T) {
			...
			cfg := baseTestConfig()
			cfg.Redaction.CustomPatterns = fixture.CustomPatterns
			if fixture.CustomRuleID != "" {
				category := fixture.CustomCategory
				if category == "" {
					category = redact.CategoryPII
				}
				cfg.Redaction.CustomPatterns = []config.CustomPattern{{ID: fixture.CustomRuleID, Category: config.PatternCategory(category), Pattern: regexp.QuoteMeta(fixture.Input), Replacement: "<CUSTOM>"}}
			}
			patterns, err := config.CustomPatternsToUserPatterns(cfg.Redaction.CustomPatterns)
			if err != nil {
				t.Fatal(err)
			}
			r, err := redact.NewRedactor(redact.Standard, patterns, redact.XDGPaths{DataHome: fixture.XDGDataHome, ConfigHome: fixture.XDGConfigHome, StateHome: fixture.XDGStateHome})
			if err != nil {
				t.Fatal(err)
			}
			if fixture.DeclaredPatterns != nil {
				// Deliberately violate the injection contract to test fail-closed metadata.
				cfg.Redaction.CustomPatterns = *fixture.DeclaredPatterns
			}
			...
			if fixture.ConfigRecorder {
				...
				pipeline, err = push.NewPipeline(store, pub, baseCreds(), cfg, fs, push.PipelineConfig{}, redactor, &stderr)
				if err != nil {
					t.Fatal(err)
				}
			}
			...
			forbidden := append([]string{originalInput, fixture.Input, fixture.InvalidCategory, fixture.InvalidRule}, fixture.Forbidden...)
			if _, reportable := fixture.Rules[fixture.CustomRuleID]; !reportable {
				forbidden = append(forbidden, fixture.CustomRuleID)
			}
			for _, pattern := range cfg.Redaction.CustomPatterns {
				forbidden = append(forbidden, pattern.Pattern, pattern.Replacement)
			}
			for _, c := range shared.Cases {
			...
			if fixture.ExpectedCode != "" {
				found := false
				for _, diagnostic := range doc.Errors {
					if diagnostic.Stage == perf.StageRedactionApply && diagnostic.Code == fixture.ExpectedCode {
						found = true
						for _, text := range fixture.DiagnosticContains {
							if !strings.Contains(diagnostic.SafeMessage, text) || !strings.Contains(trace.String(), text) {
								t.Errorf("missing recovery text %q in JSON/JSONL diagnostic: %+v", text, diagnostic)
							}
						}
					}
				}
				if !found || !strings.Contains(trace.String(), fixture.ExpectedCode) {
					t.Errorf("missing safe error code %s in JSON/JSONL: %+v", fixture.ExpectedCode, doc.Errors)
				}
			}
