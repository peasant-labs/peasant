...
func (p *Pipeline) readPublicationInput(ctx context.Context, candidate ingest.PushSessionRow) (ingest.PublicationInputBundle, error) {
	...
	if string(meta.SessionID) != candidate.SessionID || string(meta.Project.Hash) != candidate.ProjectHash ||
		...
		return input, fmt.Errorf("capture session %s for publication: %w: its recorded identity changed after selection; no content was prepared; retry the push to select its current project and harness", candidate.SessionID, ErrSessionRefused)
	}
	if p.runCfg.Repository != nil && !p.runCfg.Repository.Admits(string(meta.Project.Hash)) {
		return input, fmt.Errorf("capture session %s for publication: %w: its current project is outside the requested repository; no content was prepared; retry within the selected repository", candidate.SessionID, ErrSessionRefused)
	}
	if p.runCfg.Selection != nil {
		if p.runCfg.Selection.Decision(meta.SessionID) != ingest.BranchMatchYes ||
			...
			return input, fmt.Errorf("capture session %s for publication: %w: its project or branch no longer matches the prepared selection; no content was prepared; retry the push to refresh selection decisions", candidate.SessionID, ErrSessionRefused)
		}
	}
	return input, nil
}
