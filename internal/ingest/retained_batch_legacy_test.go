...
func TestRetainedBatchLegacyPreviewStorable(t *testing.T) {
	...
	// Mixed legacy+valid rows stay legacy preview on the preview path (the
	// sentinel proves no corruption); a corrupt follow-up still refuses and
	...
	// Mixed legacy+valid rows stay legacy preview on the store preview path:
	// the sentinel proves every record decoded and every coordinated record
	// validated with ownership agreed, so the set is bounded storage, never
	// a full certificate. Fresh-policy assessment above still refuses
	// recertification; only the durable preview write admits the set.
	...
	accepted := db.IndexSessionEntryBatch(ctx, []ingest.SessionEntryWrite{mixedWrite})
	if len(accepted) != 1 || accepted[0].Err != nil {
		t.Fatalf("mixed legacy+valid preview write refused: %+v", accepted)
	}
