...
func TestUnknownPrivateEncoding(t *testing.T) {
	...
	for _, c := range doc.Cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			assertCodecStoreRoundTrip(t, c.Extra, want, c.Payload)
		})
	}
}
...
// decodes to the same payload and still passes export certification.
func assertCodecStoreRoundTrip(t *testing.T, extra string, want int, payload string) {
	t.Helper()
	ctx := t.Context()
	...
	if _, err := ingest.ProjectRetainedUnknown(found, schema.HarnessClaudeCode); err != nil {
