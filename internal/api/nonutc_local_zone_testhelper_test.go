...
// forcedNonUTCLocalZone is the deliberately non-UTC local zone the child test
// process runs under. The offset must be non-zero for the guard to bite; the
// exact value only needs to differ from UTC.
var forcedNonUTCLocalZone = time.FixedZone("PEASANT-TEST-NON-UTC", -7*60*60)

// installForcedNonUTCLocalZoneIfRequested is called from TestMain. When the
// child re-exec environment variable is set, it replaces the process local zone
// with forcedNonUTCLocalZone BEFORE any test runs, so a test's timestamps
// serialize in that zone unless the production code normalizes them. It mutates
// the global time.Local exactly once, before m.Run, never from inside a running
// (possibly parallel) test.
func installForcedNonUTCLocalZoneIfRequested() {
	if os.Getenv(forcedNonUTCLocalZoneEnv) == "1" {
		time.Local = forcedNonUTCLocalZone
	}
}
...
//
// It is defined for the internal (package api) timestamp tests. The external
// (package api_test) timestamp tests use the identical helper in
// nonutc_local_zone_external_test.go. The two definitions must keep the same
