...
| `current-parent-navigation.mjs` | Mounted current-parent navigation evidence on the REAL binary: opens the child transcript through the real WebSocket `session_detail` payload, follows the stored `started_by` link to the exact parent, then goes Back and asserts the child's route query, earlier-history disclosure, and inner `.txn-stream` scroll offset are restored. Asserts served-chunk build provenance (the per-session reading-state marker) before any capture and writes both-theme mounted captures of the context links, the parent, and the restored child. Fixture: `internal/mock/testdata/context_navigation.yaml` (shared with the Go mock provider via `current-parent-navigation-fixture.mjs`). |
...
### 4d. Mounted current-parent navigation (real binary, both themes)
- **cmd:** `cd PEASANT/web && CHROME_PATH=$(command -v google-chrome) pnpm visual:current-parent` with §2's server cmd available (this script boots its own `bin/peasant` on `PEASANT_NAV_PORT`, default `8694`; `PEASANT_NAV_SKIP_BUILD=1` reuses an existing `bin/peasant`).
- **what:** follows the real WebSocket `session_detail` path from the child transcript to the stored `started_by` parent, then Back, and asserts the exact parent route, the restored child query, earlier-history disclosure and inner scroll offset.
- **provenance:** before capturing, the served `/_next/static/chunks` artifact must carry the host reading-state marker; a stale export fails loud.
- **outputs:** `PEASANT_NAV_CAPTURE_DIR` (default `/tmp/opencode/current-parent-navigation-captures`)/`<theme>/{child-context-links,child-scrolled-before-follow,parent-session,child-restored-after-back,child-restored-disclosure}.png` — review artifacts, never committed.
