 ...
 * small session-scoped record and the adapter replays it on the way back:
 *
 * - the inner `.txn-stream` scroll offset,
 * - the selected turn while the reader is navigating,
 * - the earlier-history disclosure map (section id -> open).
 *
 * The route query itself lives in the URL, so the browser's Back already
 * restores it; this record covers the state a URL cannot carry.
 */
...
export const EMPTY_TRANSCRIPT_READING_STATE: TranscriptReadingState = Object.freeze({
  scrollTop: 0,
  earlierHistoryOpen: Object.freeze({}) as Record<string, boolean>,
});
...
function normalizeEarlierHistoryOpen(value: unknown): Record<string, boolean> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
  const normalized: Record<string, boolean> = {};
  for (const [key, open] of Object.entries(value as Record<string, unknown>)) {
    if (key.length > 0 && typeof open === 'boolean') normalized[key] = open;
  }
  return normalized;
}
...
export function normalizeTranscriptReadingState(value: unknown): TranscriptReadingState | null {
  ...
  return {
    ...
    earlierHistoryOpen: normalizeEarlierHistoryOpen(record.earlierHistoryOpen),
  };
}
