...
Peasant ships macOS support two ways: a **Homebrew cask** (the managed channel,
published on each final release) and official **release tarballs**. Thin
per-architecture binaries are published for both Apple Silicon (`arm64`) and Intel
(`amd64`); the cask selects the right one automatically.

> **Availability:** the Homebrew cask is served from `peasant-labs/homebrew-tap` on
> final releases. Release candidates (`-rcN`) are never pushed to the tap. The
> [raw binary tarball](#current-path-raw-binary-tarball) remains available as an
> alternative and for installing release candidates.
...
not set the quarantine attribute. The Homebrew cask also removes it during
installation.

## Homebrew (cask)

The managed macOS channel. The tap is populated on each final release; release
candidates are never pushed to it.
...
| Bottle | A prebuilt formula keg that Homebrew can pour instead of building from source | Homebrew CI for official formulae; third-party taps can host their own | Peasant does not publish bottles; its cask downloads Peasant release archives directly |
| Cask | A Ruby package definition for prebuilt apps or binary artifacts | Commonly `homebrew/cask` or third-party taps | Peasant's managed macOS channel is a cask generated by GoReleaser and published to the tap on final releases |
...
A **cask** is the definition Peasant uses. It points at the Darwin archives attached
to Peasant's GitHub Release and declares how to install the `peasant` binary.
GoReleaser writes this cask into `peasant-labs/homebrew-tap` on final releases only.
Release candidates never update the tap.
...
one exists; otherwise it builds from source. Peasant's cask path skips that
...
For Peasant specifically, `peasant-labs/homebrew-tap` is the managed macOS channel,
populated on final releases. An official `homebrew/core` formula is deferred until
the source build story is acceptable to Homebrew. The open-source licensing that
`homebrew/core` policy prefers is satisfied - Peasant is Apache-2.0. If that happens,
Homebrew bottles would be produced by Homebrew's own CI; the vendor tap can either
remain as a faster-update channel or be retired.
...
- Code signing + notarization (so raw downloads "just work" with no `xattr` step)
  and a `.pkg` installer are
  [deferred](../release-runbook.md#8-deferred-ladder).
