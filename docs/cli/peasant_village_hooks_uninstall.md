...
Remove the village upload hooks Peasant installed
...
```
peasant village hooks uninstall [--event post-commit] [--event pre-push] [--dir <repo>] [flags]
```
...
```
      ...
      --event stringArray   Git event to remove; repeat for more than one (default: both)
