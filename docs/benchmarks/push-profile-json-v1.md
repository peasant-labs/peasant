...
| `rulesMatched` | Run delta of the engine's cumulative built-in and configured custom rule match counts. These are not exact replacements: overlapping matches and matches that rewrite an existing placeholder can be counted. |
...
Rule IDs from `redact.Rules`, the exact engine-generated IDs
`xdg_data_home`, `xdg_config_home`, and `xdg_state_home`, and validated configured
custom IDs can enter a profile.
`redact.NewRedactor` generates these three runtime rules from `XDGPaths`, outside
the static catalogue. Their category is `CategoryPaths`, displayed as `PATH`.
Their patterns and replacements depend on the paths; their IDs do not. No path
or replacement text enters a profile. An ID prefix alone never grants eligibility.

If the XDG directories share a matching prefix, all three rules can count a
match at the same location. These are rule **matches**, not distinct replacements;
overlaps are not deduplicated by the profiler.

Configured custom IDs are intentionally reportable diagnostic metadata, including
names such as `secret_token` or `private.pii-rule`; choose IDs suitable for sharing.
They must pass the recorder's existing lowercase diagnostic-token grammar
(`^[a-z0-9][a-z0-9_.:-]*$`) unchanged. Paths, remotes, whitespace, control characters,
and other malformed IDs are not accepted or silently normalized. Regex values,
replacements, matched content and raw config conversion errors are never reported.
The catalogue uses the same validated config conversion as engine construction.

When multiple rules use the same ID and category, their match counts are combined
under that ID, including built-in/custom and custom/custom collisions. No distinct
origin or replacement count is claimed. If the same ID maps to conflicting
categories, exact category attribution is impossible: any report containing that
ID withholds all finding/rule counts with `redaction_report_custom_collision`.
Rename the colliding custom rules to obtain attributable totals.
Production construction uses the same custom-pattern config for the engine and
pipeline. Injected engines must honor that requirement; the report alone cannot
verify undisclosed custom patterns or their categories.

Unknown/unconfigured IDs (including unconfigured XDG-prefixed names), unsafe IDs,
invalid custom config, conflicting-category ID collisions,
unknown/inconsistent categories, nonpositive or overflowing counts, inconsistent
totals, or decreasing reports withhold
...
general error sanitizer. Safe errors use fixed local descriptions and classified
codes: `redaction_report_unknown_rule`, `redaction_report_custom_collision`,
`redaction_report_unsafe_rule`, `redaction_report_config`,
`redaction_report_category`, `redaction_report_count`, `redaction_report_total`,
and `redaction_report_decreasing`. These explain that counts were withheld, not
publication, and give recovery guidance without naming the offending input.
Missing reports use `redaction_report_unavailable`. Document failures use
`redaction_entries_validation`, `redaction_metadata_validation`, or
`redaction_transcript_validation` and explain that publication was refused.
