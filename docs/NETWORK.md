...
**Failure behaviour:** a lookup that fails — no network, a non-2xx status, or a response this version cannot read — prints nothing and changes nothing about the push. It cannot publish, and it cannot spend the upload's time: the read happens before the `--timeout` clock for the upload starts, and is bounded to one second of its own. A village that accepts the connection and never answers costs that second and no more; the upload still gets its whole budget.
