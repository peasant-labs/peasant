...
OpenCode SQLite materialization separately from bounded, completion-checked index
results (the data accepted for persistence). It never opens the user's native
...
files, and the named index-completion and OpenCode native fixture families.
...
One compatibility bridge supports the former global `CurrentIndexVersion` and
adapter baseline 1. It invokes the former `IndexTranscript` persistence path
when the completion-checked result API did not exist. It does not simulate an
old parser. Other incompatible APIs or fixture schemas fail with a prerequisite
