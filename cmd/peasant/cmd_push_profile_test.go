...
type pushProfileCase struct {
	RedactionForbidden   []string         `yaml:"redactionForbidden"`
	RedactionInput       string           `yaml:"redactionInput"`
	RedactionRules       map[string]int64 `yaml:"redactionRules"`
	RedactionCategories  map[string]int64 `yaml:"redactionCategories"`
	RedactionConfig      string           `yaml:"redactionConfig"`
	RedactionFailures    int64            `yaml:"redactionFailures"`
	RedactionErrorCode   string           `yaml:"redactionErrorCode"`
...
func TestPushCmd_Profile(t *testing.T) {
	...
	for _, c := range fixtures.Cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			if c.SeedSessions {
				if c.RedactionInput != "" {
					cfg = seedUploadableSession(t, dir, fixtures.SessionID, filepath.Join(dir, "synthetic-project"))
					seedEntryCarrying(t, dir, fixtures.SessionID, strings.ReplaceAll(c.RedactionInput, "@XDG@", dir))
				} else {
			...
			if c.RedactionConfig != "" {
				data, err := os.ReadFile(cfg)
				if err != nil {
					t.Fatal(err)
				}
				if err := os.WriteFile(cfg, append(data, []byte(c.RedactionConfig)...), 0o600); err != nil {
					t.Fatal(err)
				}
			}
			if c.SeedAnnotation {
			...
			forbidden = append(forbidden, c.RedactionForbidden...)
			...
			if c.RedactionRules != nil {
				if !reflect.DeepEqual(doc.Redaction.RulesMatched, c.RedactionRules) || !reflect.DeepEqual(doc.Redaction.FindingsByCategory, c.RedactionCategories) || doc.Redaction.Failures != c.RedactionFailures {
					t.Fatalf("mounted redaction metrics: got %+v; want rules=%v categories=%v failures=%d", doc.Redaction, c.RedactionRules, c.RedactionCategories, c.RedactionFailures)
				}
				if c.RedactionErrorCode != "" {
					found := false
					for _, diagnostic := range doc.Errors {
						found = found || diagnostic.Code == c.RedactionErrorCode && strings.Contains(diagnostic.SafeMessage, "counts withheld, publication unchanged")
					}
					if !found {
						t.Fatalf("missing mounted redaction diagnostic: %+v", doc.Errors)
					}
				}
			}
			if c.Mode != "quiet" {
			...
			if c.Trace && !c.TraceFailure {
				...
				if c.RedactionRules != nil {
					assertMountedRedactionTrace(t, events, doc.Redaction, c.RedactionErrorCode)
				}
...
func assertMountedRedactionTrace(t *testing.T, events []perf.Event, want perf.RedactionMetrics, code string) {
	t.Helper()
	rules, categories := map[string]int64{}, map[string]int64{}
	var failures int64
	found := code == ""
	for _, event := range events {
		if event.Kind == perf.EventKindError && event.SafeError != nil && event.SafeError.Code == code {
			found = strings.Contains(event.SafeError.SafeMessage, "counts withheld, publication unchanged")
		}
		...
		case perf.CounterRedactionRulesMatched:
			rules[event.Attributes[perf.AttrRuleID]] += event.CounterValue
		case perf.CounterRedactionFindings:
			categories[event.Attributes[perf.AttrCategory]] += event.CounterValue
		case perf.CounterRedactionFailures:
			failures += event.CounterValue
		}
	}
	if !found || !reflect.DeepEqual(rules, want.RulesMatched) || !reflect.DeepEqual(categories, want.FindingsByCategory) || failures != want.Failures {
		t.Fatalf("mounted JSONL differs from JSON: rules=%v categories=%v failures=%d diagnostic=%v", rules, categories, failures, found)
	}
}
